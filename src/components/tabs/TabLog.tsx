...
export default function TabLog() {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'hari_ini' | 'lengkap'>('hari_ini');
  ...
  // Auto fetch Presensi Terkini on mount or when date changes
  useEffect(() => {
    if (!pegawai) return;
    if (activeSubTab === 'hari_ini') {
      fetchHariIni(hariIniDate);
    }
  }, [activeSubTab, hariIniDate, pegawai]);

  if (!pegawai) {
    return <RequireLogin tabName="Cek Log Presensi" onGoToLogin={() => setActiveTab('tabLogin')} />;
  }
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 relative z-30">
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                  Log aktivitas dan foto absensi tanggal terpilih
                </p>
              </div>
        ...
        {activeSubTab === 'lengkap' && (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                Pilih rentang tanggal untuk melihat riwayat absensi
              </p>
            </div>
                ...
                >
                  ...
                  <tbody>
                    {dataLengkap.slice(0, visibleCountLengkap).map((item, i) => {
                      ...
                      else if (statusRaw === 'DK') keterangan = 'Dinas Khusus';
                      else if (statusRaw === 'C') keterangan = 'Cuti';
                      else if (statusRaw === 'I') keterangan = 'Izin';
                      else if (statusRaw === 'S') keterangan = 'Sakit';
                      else if (statusRaw === 'DL') keterangan = 'Dinas Luar';
                      else if (statusRaw === 'TL') keterangan = 'Tugas Luar';
