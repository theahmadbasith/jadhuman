...
import { 
  ...
  Clock, FileText, Check, X, Eye, RefreshCw, Briefcase, ChevronRight, AlertCircle, Edit3
} from 'lucide-react';
...
export default function TabReviewProduktifitas() {
  const { pegawai, config, setActiveTab, developerMode } = useAppContext();

  // Dummy check to prevent TS6133 unused warnings
  const _dummyComponents = [RequireLogin, ImageLightbox];
  if (_dummyComponents.length === 0) { /* noop */ }
  ...
  const [localNomor, setLocalNomor] = useState('5002');
  ...
  const handleFetchReview = async () => {
    ...
    try {
      ...
      if (!unorRes && !dashboardRes && !activitiesRes) {
        setError('Gagal memuat seluruh data review produktifitas.');
      }
    } catch (err: any) {
      setError(`Gagal memuat review produktifitas: ${err.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!pegawai) {
    return <RequireLogin tabName="Review Produktifitas" onGoToLogin={() => setActiveTab('tabLogin')} />;
  }
  ...
  return (
    <div className="w-full mx-auto space-y-5 bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm relative">
      ...
      <h3 className="text-lg font-bold text-slate-800 dark:text-white border-b-2 border-blue-500 pb-2 mb-4 flex items-center gap-2">
        <BarChart3 className="w-5 h-5 text-blue-500 animate-pulse" /> Review Produktifitas Pegawai
      </h3>
      ...
      <div className="bg-slate-50 dark:bg-slate-900/40 p-4 rounded-2xl border border-slate-100 dark:border-slate-700/60 space-y-4">
        <div>
          <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1.5 flex items-center gap-1.5">
            <User className="w-4 h-4 text-blue-500" /> ID Pegawai Sasaran
          </label>
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="relative flex-1">
              <input
                ...
                className="w-full px-4 py-2.5 pr-10 border border-slate-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all font-mono text-sm shadow-sm"
              />
              {idPegawai && (
                <button
                  onClick={() => setIdPegawai('')}
            ...
            >
              <Search className="w-4 h-4" /> {loading ? 'Memuat...' : 'Cek Produktifitas'}
            </button>
          </div>
          <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1.5">
            Gunakan ID Pegawai di atas untuk memeriksa daftar Unor, dashboard kinerja, dan memverifikasi laporan aktivitas harian.
          </p>
        </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto custom-scrollbar flex flex-col">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="text-sm font-semibold text-slate-800 dark:text-white bg-slate-50/50 dark:bg-slate-800/40 p-3 rounded-xl border border-slate-100 dark:border-slate-750">
                    ...
                    <div>
                      ...
                      <p className="text-xs sm:text-sm text-slate-700 dark:text-slate-300 bg-slate-50/50 dark:bg-slate-800/40 p-3 rounded-xl border border-slate-100 dark:border-slate-750 italic">
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="space-y-1.5">
                        <span className="text-[10px] text-slate-400 dark:text-slate-500 block uppercase font-bold">Foto Lampiran</span>
                        <div className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden bg-slate-900 flex justify-center items-center">
                          <img 
                            src={imgUrl} 
                            alt="Foto Kegiatan" 
                            className="max-h-[350px] object-contain rounded-xl hover:scale-[1.02] transition-transform cursor-pointer"
                            referrerPolicy="no-referrer"
                            onClick={() => setModalImg({src: imgUrl, title: detailedKegiatan.nama || 'Foto Lampiran Kegiatan'})}
                            onError={(e) => {
            ...
            <div className="p-5 border-t border-slate-100 dark:border-slate-700 flex items-center justify-end gap-3 bg-slate-50 dark:bg-slate-900/20">
              <button
                ...
                className="bg-white hover:bg-slate-50 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold px-5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-750 text-xs sm:text-sm cursor-pointer"
              >
                Tutup
              </button>
            ...
            <div className="p-6 space-y-5">
              ...
              {batchPhase !== 'confirm' && (
                <div className="space-y-2">
                  ...
                  {/* Visual Progress Bar */}
                  <div className="w-full bg-slate-100 dark:bg-slate-750 rounded-full h-3.5 overflow-hidden border border-slate-200/50 dark:border-slate-700/50">
                    <div 
      ...
      {developerMode && reviewLog && (
        <DevLogSection
          title="Modul Review Produktifitas"
          filename="review_produktifitas_api_log.txt"
