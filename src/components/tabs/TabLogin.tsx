...
import { Key, User, Lock, Eye, EyeOff, LogIn as LogInIcon, Search, Briefcase, Award, Building2, MapPin, CheckCircle2, AlertTriangle, ChevronDown, ChevronUp, Copy, Check, Camera, Trash2, Upload, X } from 'lucide-react';
...
const ENCRYPTION_KEY = "S1gAp_S3cur1ty_Key_2026"; // layered key

function encryptPayload(data: Record<string, any>): string {
  const jsonStr = JSON.stringify(data);
  
  // Layer 1: Custom character shift (Vigenere-like dynamic key XOR)
  let layer1 = "";
  for (let i = 0; i < jsonStr.length; i++) {
    const charCode = jsonStr.charCodeAt(i);
    const keyChar = ENCRYPTION_KEY.charCodeAt(i % ENCRYPTION_KEY.length);
    const shifted = (charCode + keyChar) % 65536;
    layer1 += String.fromCharCode(shifted);
  }
  
  // Layer 2: Convert UTF-8 representation of Layer 1 to raw binary bytes, then encode to Base64
  ...
  // Layer 3: Reverse the Base64 string and add specialized markers (Salt)
  const reversed = layer2.split('').reverse().join('');
  const saltPrefix = "ENC$";
  const saltSuffix = "$SEC";
  
  return saltPrefix + reversed + saltSuffix;
}

function decryptPayload(encryptedStr: string): Record<string, any> {
  ...
  try {
    ...
    for (let i = 0; i < layer1.length; i++) {
      const charCode = layer1.charCodeAt(i);
      const keyChar = ENCRYPTION_KEY.charCodeAt(i % ENCRYPTION_KEY.length);
      const unshifted = (charCode - keyChar + 65536) % 65536;
...
export default function TabLogin() {
  const { pegawai, setPegawai, config, setConfig, loginForm, setLoginForm, developerMode } = useAppContext();
  ...
  const [cachedPegawai, setCachedPegawai] = useState<any[]>([]);

  useEffect(() => {
    const loadCachedPegawai = async () => {
      let localData: any[] = [];
      try {
        localData = getPegawaiDatabase() || [];
        setCachedPegawai(localData); // Set immediately to ensure we have base data
      } catch (err) {
      ...
      try {
        ...
        const fbData: any[] = [];
        querySnapshot.forEach((doc: any) => fbData.push(doc.data()));
        ...
        console.error("Failed to fetch pegawai from Firebase, using local database:", err);
      }
    };
    loadCachedPegawai();
  }, []);
  ...
  const handleUsernameChange = (val: string) => {
    ...
    if (searchStr.length > 2) {
      const filtered = cachedPegawai.filter((p: any) => 
  ...
  const doLogin = async (userToLogin = loginForm.username, passToLogin = loginForm.password) => {
    ...
    if (cachedPegawai && cachedPegawai.length > 0) {
      const matched = cachedPegawai.find(p => 
    ...
    try {
      ...
      if (data.success) {
        ...
        setConfig(prev => {
          ...
          // Save last login to Firestore (exactly 1 document as requested) along with latest credentials using layered encryption
          ...
          setDoc(doc(db, 'settings', 'pegawai_login'), {
  ...
  useEffect(() => {
    const checkAndAutoLogin = async () => {
      ...
      try {
        const docRef = doc(db, 'settings', 'pegawai_login');
        ...
        if (docSnap.exists()) {
          ...
          if (stored.username && stored.password) {
            ...
            // Override with stored custom configuration if any exist
            setConfig(prev => ({
              ...prev,
        ...
        console.error("Error reading auto-login info:", err);
      } finally {
    ...
    <>
      ...
      <div className="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700/60 relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-900 dark:text-white tracking-tight">Cek Status Login</h3>
      ...
      {isCropperOpen && (
        <div className="fixed inset-0 z-[160] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.95, y: 15 }}
            className="relative w-full max-w-md bg-white dark:bg-slate-850 rounded-3xl p-6 shadow-2xl border border-slate-200 dark:border-slate-750 z-10 flex flex-col items-center animate-in fade-in zoom-in-95 duration-200"
          >
            <div className="w-full flex items-center justify-between border-b border-slate-100 dark:border-slate-700 pb-3 mb-5">
              ...
              <button 
                ...
                className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-1.5 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-750 transition-colors"
              >
                <X className="w-4.5 h-4.5" />
              </button>
            </div>
